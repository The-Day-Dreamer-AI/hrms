...
import ClaimTypesSelector from "@/components/claim-types-selector";
...
import { columns } from "../claim/columns";
...
export default function ClaimsReport() {
  ...
  const { mutateAsync: onSetProcessed, isPending: isProcessing } = useMutation({
    ...
    mutationFn: async (ids: string[]) => {
      const res = await client.post("/api/user-claims/process-payment", { ids });
      return res;
    },
    onError(e) {
      toast({
  ...
  const { refetch, isLoading } = useQuery({
    queryKey: ["claims-reports-export"],
    enabled: false,
    queryFn: async () => {
      const res = await client.get("/api/reports/export-claims", {
  ...
  const {
    ...
    enabled: false,
    queryKey: ["claim", pagination, sorting],
    queryFn: async () => {
      const res = await client.get("/api/reports/claims", {
        params: queryParams(),
      });
  ...
  // Get the selected claim IDs from the row selection
  const getSelectedClaimIds = () => {
    if (!queryData?.data || Object.keys(rowSelection).length === 0) return [];
    
    return Object.keys(rowSelection).map(rowId => {
      const selectedRow = queryData.data[parseInt(rowId)];
      return selectedRow?.id?.toString();
    }).filter(Boolean);
  };
  ...
  return (
    <div className="w-full flex flex-col space-y-4">
      <div className="flex flex-col space-y-1">
        <ModuleTitle>Claims Report</ModuleTitle>
      ...
      <Card className="gap-2 flex flex-wrap p-2 sm:px-4 items-center">
        ...
        />
        <ClaimTypesSelector onChange={setClaim} value={claim} />
